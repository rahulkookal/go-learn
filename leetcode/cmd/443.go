...
chars[i] is a lowercase English letter, uppercase English letter, digit, or symbol.`,
	Run: func(cmd *cobra.Command, args []string) {
		fmt.Println("p443 called")
		input := []string{"a", "a", "b", "b", "c", "c", "c"}
		joined := strings.Join(input, "")
		in := []byte(joined)
		result := solutions.Compress(in)
		fmt.Println("compressed: ", result)
	},
}

func init() {
	rootCmd.AddCommand(p443Cmd)

	// Here you will define your flags and configuration settings.

	// Cobra supports Persistent Flags which will work for this command
	// and all subcommands, e.g.:
	// p443Cmd.PersistentFlags().String("foo", "", "A help for foo")

	// Cobra supports local flags which will only run when this command
	// is called directly, e.g.:
	// p443Cmd.Flags().BoolP("toggle", "t", false, "Help message for toggle")
}
